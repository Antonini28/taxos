...
**A second tax type, authored not engineered.** UK Corporation Tax computes through the *same* deterministic engine, persistence, lineage and audit trail as VAT — the adjustment of accounting profit to taxable total profits, and the main-rate charge, live entirely in a rule pack. Adding it exposed the one place the engine still hardcoded arithmetic (the VAT return's derived boxes); that was generalised into a formula evaluator, VAT output proven byte-identical, and Corporation Tax then dropped in as pure content. That is the platform's central claim (AP-3) made real, not asserted.

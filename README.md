...
**A second tax type, authored not engineered.** UK Corporation Tax runs the *entire* governed pipeline that VAT does — the same deterministic engine, lineage, segregation-of-duties approval, hash-chained audit, and one-click evidence pack — with nothing tax-specific outside a rule pack. The adjustment of accounting profit to taxable total profits and the main-rate charge are content; the approval gate and the audit-ready download needed no CT-specific code at all. Adding CT exposed the one place the engine still hardcoded arithmetic (the VAT return's derived boxes); that was generalised into a formula evaluator, VAT output proven byte-identical, and Corporation Tax then dropped in. That is the platform's central claim (AP-3) made real, not asserted.

...
**Built and running (R1):** ingestion, the deterministic engine with lineage (UK VAT **and** UK Corporation Tax, on one engine via rule packs), the agent runtime, the approval gate, the audit chain, the fraud & risk centre with an explainable Rung-2 risk model (Phase 5), the one-click evidence pack, the grounded knowledge/research layer (Phase 4), and the executive dashboard — with tests, migrations, and a one-command demo.

**Designed, not yet built:** the supervised layers of the ML estate (Rung 3+ — dispositions-as-labels, the GBM stack), Azure deployment (Phase 8), and the remaining screens. Each is specified to implementation depth in its phase documents — the roadmap in [docs/discovery/05-roadmap.md](docs/discovery/05-roadmap.md) is the honest sequence.
